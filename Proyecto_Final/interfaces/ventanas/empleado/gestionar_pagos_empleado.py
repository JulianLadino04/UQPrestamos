...
class gestionar_pagos_empleado:
    def __init__(self):
        ...
        # Título
        title_label = ctk.CTkLabel(main_frame, text="Pagos del Sistema", font=("Arial", 24, "bold"), text_color="#FFFFFF")
        title_label.pack(pady=(20, 10))

        # Configurar la conexión a Oracle y obtener datos
        try:
        ...
        # Añadir botones
        ctk.CTkButton(botones_frame, text="Pagar Préstamo", command=self.obtener_seleccion, font=("Arial", 14, "bold"), width=150).grid(row=0, column=0, padx=10, sticky="ew")
        ...
        # Configuración para expansión equitativa de botones
    ...
    def obtener_seleccion(self):
        """Obtiene la fila seleccionada y realiza acciones."""
        selected_item = self.tree.selection()
        if selected_item:
            fila = self.tree.item(selected_item)['values']
            print(f"Pago seleccionado: {fila}")
            # Aquí abrirías la lógica de pagar préstamo
            self.root.destroy()
            realizar_pago_prestamo = RealizarPagoPrestamo(fila[0])  # Pasa el ID del préstamo
            realizar_pago_prestamo.root.mainloop()
        else:
            print("No se ha seleccionado ninguna fila")

    def ir_a_opciones(self):
        """Ir a la ventana de opciones."""
        self.root.destroy()
